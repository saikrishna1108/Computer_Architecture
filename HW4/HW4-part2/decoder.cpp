...
;
    ...
    if (!(getOpcode(inst) ^ 0b01100111)) {
     ...
     if (!(getOpcode(inst) ^ 0b01010011)) {
        if (!(getfun7(inst) ^ 0b00000000)) { opcodeN = "fadd.s"; };
        if (!(getfun7(inst) ^ 0b00000100)) { opcodeN = "fsub.s"; };
    }
...
string check_type(string opcode) {
    ...
    "divw", "divuw", "remw", "remuw", "and", "or", "xor", "sll", "srl", "sra", "sllw", "srkw", "sraw", "slt", "sltu","fadd.s","fsub.s" };
